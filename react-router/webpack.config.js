...
module.exports = {
    ...
    entry: {
        app: ['./client'], 
        // 근데 clients.jsx에서 이미 MineSearch를 불러오고 있다. 이런걸 webpack이 알아서 다 파악한다.(뭐가 뭐를 불러오는지)
        // 그래서 이미 다른 파일이 불러오는 파일은 적어줄 필요가 없다. MineSearch를 안적어줘도 된다.) 파일 쪼갤 때 사용하던
        // react랑 react-dom도 같이 불러오고 !!🟢

    }, // 입력, 목표: client, MineSearch를 app.js 파일 하나 만들어서 html이 실행할 수 있게 해주는 것. 
    ...
    // 이후에 터미널에 성공 메세지 확인이 가능하다. asset app.js [emitted] client.jsx MineSearch[built]😀
    ...
    devServer: { // webpack이 1년 단위로 대대적으로 바뀐다 한다🤣, webpack-dev-server 4버전과 react-refresh-webpack-plugin 5버전의 수정사항
        ...
        // react-router에서 새로고침 할 떄 cannot get 에러가 뜨는데 그것을 꼼수처럼 해결 ! 

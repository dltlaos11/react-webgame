...
module.exports = {
    ...
    entry: {
        app: ['./client'], 
        // 근데 clients.jsx에서 이미 TicTacToe를 불러오고 있다. 이런걸 webpack이 알아서 다 파악한다.(뭐가 뭐를 불러오는지)
        // 그래서 이미 다른 파일이 불러오는 파일은 적어줄 필요가 없다. TicTacToe를 안적어줘도 된다.) 파일 쪼갤 때 사용하던
        // react랑 react-dom도 같이 불러오고 !!🟢

    }, // 입력, 목표: client, TicTacToe를 app.js 파일 하나 만들어서 html이 실행할 수 있게 해주는 것. 
    ...
    // 이후에 터미널에 성공 메세지 확인이 가능하다. asset app.js [emitted] client.jsx TicTacToe[built]😀
